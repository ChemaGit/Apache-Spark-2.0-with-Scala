...
object DataFrames {
  ...
  case class EmployeeW(id: Int,name: String,jobTitle: String)
  case class Account(balance: Double,employees:Array[EmployeeW])
  case class Project(title: String,team: Array[EmployeeW],acct: Account)
  /*
  StructType(
  ...
  */
  ...
  def main(args: Array[String]): Unit = {
    ...
    val tuplePerson = sc
      ...
      .map(line => line.split(","))
      .map(f => new Person(f(0).toInt, f(1), f(2), f(3)))
    ...
    val employeeDF = sc
      .textFile("hdfs://quickstart.cloudera/user/cloudera/files/employee.csv")
      .map(line => line.split(","))
      .map(f => new Employee(f(0).toInt, f(1), f(2), f(3).toInt,f(4)))
      .toDF
    ...
    /**
      ...
      */
    val over30 = employeeDF
      .filter("age > 30")
      .show()
    val over30W = employeeDF
      .where("age > 30")
      .show()

    val filterComplex = employeeDF
      .filter(($"age" > 25) && ($"city" === "Sydney"))
      .show()
    ...
    /**
      ...
      */
    val listingsDF = sc
      .textFile("hdfs://quickstart.cloudera/user/cloudera/files/listings.csv")
      .map(line => line.split(","))
      .map(f => new Listing(f(0), f(1).toInt, f(2).toInt))
      .toDF

    val mostExpensive = listingsDF
      .groupBy($"zip")
      .max("price")

    val lessExpensive = listingsDF
      .groupBy($"zip")
      .min("price")
    ...
    val postsDF = sc
      .textFile("hdfs://quickstart.cloudera/user/cloudera/files/posts.csv")
      .map(line => line.split(","))
      .map(f => new Post(f(0).toInt, f(1), f(2).toInt, f(3)))
      .toDF
    ...
    val abosWithOptionalLocationsDF = abosDF
      .join(locationsDF, abosDF("id") === locationsDF("id"), "left-outer")
    ...
    /**
      ...
      */
    val demographicsDF = sc
      .textFile("hdfs://quickstart.cloudera/user/cloudera/files/demographic.csv")
      .map(line => line.split(","))
      .map(d => (new Demographic(d(0).toInt,d(1).toInt,d(2).toBoolean,d(3),d(4),d(5).toBoolean,d(6).toBoolean)) )
      .toDF

    val financesDF = sc
      .textFile("hdfs://quickstart.cloudera/user/cloudera/files/finances.csv")
      .map(line => line.split(","))
      .map(f => (new Finances(f(0).toInt,f(0).toBoolean,f(0).toBoolean,f(0).toBoolean,f(0).toInt)))
      .toDF

    val filtered = financesDF
      .filter($"hasDebt" && $"hasFinancialDependents")

    val joined = demographicsDF
      .filter($"country" === "Switzerland")
      .join(filtered, demographicsDF("id") === filtered("id"), "inner")
      .count
