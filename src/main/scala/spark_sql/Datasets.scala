...
object Datasets {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val listingsDF = sc
      .textFile("hdfs://quickstart.cloudera/user/cloudera/files/listing.csv")
      .map(line => line.split(","))
    ...
    val avgDS = listingsDS
      .groupByKey(l => l.zip) // looks like groupByKey on RDDs
      .agg(avg($"price")
      .as[Double]) // looks like our DataFrame operators!
    ...
    val myDS = spark
      .read
      .json("hdfs://quickstart.cloudera/user/cloudera/files/people.json")
      .as[Person]
    ...
    keyValuesDS
      .groupByKey(p => p._1).mapGroups((k, vs) => (k, vs.foldLeft("")((acc, p) => acc + p._2)))
      .sort($"_1")
      .show()
    //+---+----------+
    //| _1|       _2 |
