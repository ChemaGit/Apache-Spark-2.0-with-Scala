...
object StructureAndOptimization {
  ...
  lazy val spark = SparkSession
    .builder()
    .appName("Structure and Optimization")
    .master("local[*]")
    .getOrCreate()
  ...
  def main(args: Array[String]): Unit = {
    ...
    val demographics = sc
      .textFile("hdfs://quickstart.cloudera/user/cloudera/files/demographics.csv")
      .map(line => line.split(","))
      .map(d => (d(0).toInt, new Demographic(d(0).toInt,d(1).toInt,d(2).toBoolean,d(3),d(4),d(5).toBoolean,d(6).toBoolean)) )

    val finances = sc
      .textFile("hdfs://quickstart.cloudera/user/cloudera/files/finances.csv").map(line => line.split(","))
    ...
    demographics
      .join(finances)
      .filter({case(k,(d, f)) => d.country == "Switzerland" && f.hasFinancialDependents && f.hasDebt })
      .count
    ...
    val filtered = finances
      .filter({case(k, v) => v.hasFinancialDependents && v.hasDebt})
    demographics
      .filter({case(k, v) => v.country == "Switzerland"})
      .join(filtered)
      .count
    ...
    cartesian
      ...
      .filter({case(p1, p2) => p1._2.country == "Switzerland" && p2._2.hasFinancialDependents && p2._2.hasDebt })
      .count
