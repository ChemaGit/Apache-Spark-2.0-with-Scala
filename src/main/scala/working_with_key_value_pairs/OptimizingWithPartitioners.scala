...
object OptimizingWithPartitioners {
  ...
  def main(args: Array[String]): Unit = {
    val spark = SparkSession
      .builder()
      .appName("Optiminzing with Partitioners")
      .master("local[*]")
      .getOrCreate()
    ...
    /**
      ...
      */
        ...
        val userData = sc.textFile("hdfs://quickstart.cloudera/user/cloudera/files/userData.csv")
          .map(line => line.split(","))
          .map(r => (r(0).toInt,r(1)))
          .persist()

        def processNewlogs(logFileName: String, userData:RDD[(Int, String)] ) {
          val events = sc.textFile(logFileName)
            .map(line => line.split(","))
            .map(r => (r(0).toInt,r(1)))
          val joined = userData.join(events) //ROD of (UserID, (UserInfo, LinkInfo))
          joined.toDebugString
          val offTopicVisits = joined.filter {case (userId, (userInfo, linkInfo)) => //Expand the tuple
                  !userInfo.equals(linkInfo)
          }.count()
        ...
        */
        val userDataB = sc.textFile("hdfs://quickstart.cloudera/user/cloudera/files/events.csv")
          .map(line => line.split(","))
          .map(r => (r(0).toInt,r(1)))
