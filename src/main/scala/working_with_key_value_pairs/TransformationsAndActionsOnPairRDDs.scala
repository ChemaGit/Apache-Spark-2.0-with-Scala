...
object TransformationsAndActionsOnPairRDDs {
  ...
  def main(args: Array[String]) {
    val spark = SparkSession
        .builder()
        .appName("Transformations And Actions On Pair RDDs")
        .master("local[*]")
        .getOrCreate()
    ...
    val visits = sc.textFile("hdfs://quickstart.cloudera/user/cloudera/files/visits.csv")
      .map(line => line.split(","))
      .map(f => (f(0), 1))
    //val numUniqueVisits = visits.reduceByKey({case(v, v2) => (v + v2)})
    val numUniqueVisits = visits
      .keys
      .distinct()
      .count()

    println(s"num Unique Visits: $numUniqueVisits")
    println("*********************")
