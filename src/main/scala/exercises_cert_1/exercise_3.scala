...
val products = sc.textFile("/user/cloudera/files/product.csv").map(line => line.split(",")).filter(arr => arr(0) != "productID").map(arr => (arr(0).toInt,arr(1),arr(2),arr(3).toInt,arr(4).toFloat)).toDF("id","code","name","quantity","price")
